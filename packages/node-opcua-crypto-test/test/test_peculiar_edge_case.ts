...
const _ok_privateKey = `-----BEGIN PRIVATE KEY-----
 ...
 */
let _crypto: typeof nativeCrypto | PeculiarWebCrypto | undefined;

declare const crypto: typeof nativeCrypto | PeculiarWebCrypto;
...
if (typeof window === "undefined") {
    ...
    x509.cryptoProvider.set(_crypto as unknown as Crypto);
} else {
...
function getCrypto(): Crypto {
    return (_crypto as Crypto) || crypto;
}
...
async function _generatePrivateKeyFileJSRSA(privateKeyFilename: string, modulusLength: 2048 | 3072 | 4096) {
...
async function generatePrivateKeyPem(modulusLength: 2048): Promise<string> {
    ...
    const { privPem, privDer: _privDer } = await privateKeyToPEM(cryptoKey);
    return privPem;
}
...
async function testStuff() {
    ...
    const _publicKey = await buildPublicKey(privateKey);
...
describe("Test @peculiar/x509 (investigate github node-opcua issue#1289 ", { timeout: 1000000 }, () => {
    ...
    it.skip("T4 - messing with Private Key", async () => {
        let success = 0;
        for (let i = 0; i < 100; i++) {
            //
    ...
    it.skip("T5 - messing with Private Key directly", async () => {
        let success = 0;
        for (let i = 0; i < 100; i++) {
            //
            if (!(i % 10)) console.log("i=", i);
